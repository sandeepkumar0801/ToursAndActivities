...
{
    public class FareHarborCriteriaServiceTest : BaseTest
    {
        private IFareHarborCriteriaService _fareHarborCriteriaService;
        ...
        public void TestInitialise()
        {
            ...
            {
                _fareHarborCriteriaService = scope.Resolve<IFareHarborCriteriaService>();
            }
        }
        ...
        public void GetAvailabilityAndServiceDetailsTest()
        {
            var products = new List<IsangoHBProductMapping>
            {
                new IsangoHBProductMapping
                {
                    ...
                    MinAdultCount = 1,
                    FactSheetId = 26941,
                    HotelBedsActivityCode = "onlocationtoursnewyork",
                    IsangoHotelBedsActivityId = 13527,
                    IsangoRegionId = 7235,
                }
            };
            var criteria = CreateCriteria(products);
            var availability = _fareHarborCriteriaService.GetAvailability(criteria);

            var serviceDetails = _fareHarborCriteriaService.GetServiceDetails(availability, products);
        ...
        public void GetAvailabilityAndServiceDetails_NegativeTest()
        {
            ...
            var serviceDetails = _fareHarborCriteriaService.GetServiceDetails(activities, products);
            Assert.IsNotNull(serviceDetails);

            //Null Check Scenario
            products.First().IsangoHotelBedsActivityId = 1234;
            serviceDetails = _fareHarborCriteriaService.GetServiceDetails(availability, products);
            Assert.IsNotNull(serviceDetails);

            //Null Check Scenario
            criteria.Counter = 0;
            availability = _fareHarborCriteriaService.GetAvailability(criteria);
