...
{
    public class HBBookingCancelCmdHandler : CommandHandlerBase, IHbBookingCancelCmdHandler
    {
        public HBBookingCancelCmdHandler(ILogger iLog) : base(iLog)
        {
        }

        protected override object CreateInputRequest<T>(T hotelbedCriteriaApitude)
        {
            var inputContext = hotelbedCriteriaApitude as InputContext;
        ...
        protected override async Task<object> GetResultsAsync(object input)
        {
            ...
            var paramflag = Constant.CancellationFlagValue;
