...
{
    public class DecideCommandHandler : CommandHandlerBase, IDecideCommandHandler
    {
        public DecideCommandHandler(ILogger log) : base(log)
        {
        }

        protected override object CreateInputRequest<T>(T inputContext, object requestExtraData, string token)
        {
            var riskifiedOrder = new RiskifiedEntities.RiskifiedDecideRequest();
            try
            {
                ...
                if (booking != null)
                {
                    ...
                    {
                        //var quantityBooked = selectedProduct.AgeGroupDescription.Where(x => x.PassengerType != PassengerType.Infant.ToString())?.Sum(x => x.PaxCount) ?? 0;
                    ...
                    riskifiedOrder = new RiskifiedEntities.RiskifiedDecideRequest()
                    {
                        Order = new RiskifiedEntities.OrderData()
                        {
