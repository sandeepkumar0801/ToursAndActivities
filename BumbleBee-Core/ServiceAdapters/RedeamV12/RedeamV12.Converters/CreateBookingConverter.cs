...
using ServiceAdapters.RedeamV12.RedeamV12.Entities.CreateBooking;
...
namespace ServiceAdapters.RedeamV12.RedeamV12.Converters
{
    public class CreateBookingConverter : ConverterBase, ICreateBookingConverter
    {
        ...
        {
            var redeamSelectedProduct = (CanocalizationSelectedProduct)selectedProduct;
            redeamSelectedProduct.BookingReferenceNumber = createBookingResponse.Booking.Id.ToString();
            redeamSelectedProduct.QrCodes = new List<Isango.Entities.Canocalization.QrCode>();
            if (createBookingResponse?.Booking?.Tickets != null)
            {
                foreach (var ticket in createBookingResponse.Booking.Tickets)
                {
                    ...
                    var qrCode = new Isango.Entities.Canocalization.QrCode
                    {
