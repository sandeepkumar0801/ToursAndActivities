...
{
    public class GetSupplierCommandHandler : CommandHandlerBase, IGetSupplierCommandHandler
    {
        public GetSupplierCommandHandler(ILogger iLog) : base(iLog)
        {
        }

        protected override object AotApiRequest<T>(T inputContext)
        {
            var supplierInfoRequest = inputContext as SupplierInfoRequest;
            if (supplierInfoRequest == null) return null;

            supplierInfoRequest.AgentID = AgentId;
            supplierInfoRequest.Password = Password;

            HttpClient.DefaultRequestHeaders.AcceptEncoding.Add(new System.Net.Http.Headers.StringWithQualityHeaderValue("gzip"));

            var content = new StringContent(SerializeXml(supplierInfoRequest), Encoding.UTF8, Constant.ApplicationMediaType);
