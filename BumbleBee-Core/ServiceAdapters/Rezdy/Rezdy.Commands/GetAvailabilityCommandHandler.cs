...
namespace ServiceAdapters.Rezdy.Rezdy.Commands
{
    public class GetAvailabilityCommandHandler : CommandHandlerBase, IGetAvailabilityCommandHandler
    {
        ...
        {
            var input = inputContext as AvailabilityRequest;
        ...
        private string GenerateMethodPath(AvailabilityRequest input)
        {
            return string.Format(UriConstants.Availability, input.ProductCode, input.StartDate.ToString("yyyy-MM-dd HH:MM:ss"), input.EndDate.ToString("yyyy-MM-dd HH:MM:ss"), input.Limit,input.Offset);
        }
    }
}