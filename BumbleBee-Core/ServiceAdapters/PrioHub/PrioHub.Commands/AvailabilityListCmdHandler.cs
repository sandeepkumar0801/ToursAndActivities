...
{
    public class AvailabilityListCmdHandler : CommandHandlerBase, IAvailabilityListCommandHandler
    {
        public AvailabilityListCmdHandler(ILogger iLog) : base(iLog)
        {
        }
        ...
        {
            var result = default(AvailabilityListResponse);
            try
            {
                result = SerializeDeSerializeHelper.DeSerialize<AvailabilityListResponse>(responseText);
            }
            catch (Exception ex)
            {
                //ignored
