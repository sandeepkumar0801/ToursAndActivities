...
{
    public class RouteCommandHandler : CommandHandlerBase, IRouteCommandHandler
    {
        public RouteCommandHandler(ILogger iLog) : base(iLog)
        {
        }
        ...
        protected override object GetResponseObject(string responseText)
        {
            ...
            try
            {
                result = SerializeDeSerializeHelper.DeSerialize<RouteResponse>(responseText);
            }
            catch (Exception ex)
            {
                //ignored
        ...
        protected override object GetResultsAsync(object input)
        {
            ...
            //products/routes?distributor_id =501&items_per_page=100
