...
{
    public class GetActivityConverter : ConverterBase, IGetActivityConverter
    {
        public GetActivityConverter(ILogger logger) : base(logger)
        {
        }
        ...
        /// <param name="objectResult">Generic model for Get Activity Call</param>
        /// <param name="criteria">Generic request model</param>
        /// <returns></returns>
        public object Convert<T>(T objectResult, T criteria)
        {
            var result = SerializeDeSerializeHelper.DeSerialize<GetActivityRs>(objectResult.ToString());
