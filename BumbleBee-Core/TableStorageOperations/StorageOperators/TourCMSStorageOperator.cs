...
{
    public class TourCMSStorageOperator : BaseStorageOperator
    {
        /// <summary>
        ...
        public override void InsertAvailabilitiesData(Activity activity, string tokenId)
        {
            ...
            foreach (var productOption in activity.ProductOptions)
            {
                ...
                foreach (var date in validDates)
                {
                    ...
                    var gtEntity = new TourCMSAvailabilities
                    {
                        ...
                        PickupPointsDetails = SerializeDeSerializeHelper.Serialize(actOpt?.PickupPointsForTourCMS),
                        ContractQuestions = SerializeDeSerializeHelper.Serialize(actOpt?.ContractQuestions),
                        PartitionKey = tokenId,
                        RowKey = rowKey,
                        RateKey = actOpt.RateKey
                    };
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            TourCMSAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new TourCMSAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
