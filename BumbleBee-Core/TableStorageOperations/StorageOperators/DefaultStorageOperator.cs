...
{
    public class DefaultStorageOperator : BaseStorageOperator
    {
        /// <summary>
                ...
                foreach (var productOption in productOptions)
                {
                    ...
                    foreach (var date in validDates)
                    {
                        try
                        {
                            ...
                            var aotEntity = new DefaultAvailabilities
                            {
                            ...
                            try
                            {
                                //var test = (ActivityOption)productOption;
                                if (activity?.ApiType == Isango.Entities.Enums.APIType.Undefined && (activity?.IsServiceLevelPickUp ?? false) && activity.PickUpOption == Isango.Entities.PickUpDropOffOptionType.MultiplePickupLocation)
                                {
                                    aotEntity.PickupLocations = SerializeDeSerializeHelper.Serialize(activity?.PickupLocations);
                                }
                                else if (activity?.ApiType == Isango.Entities.Enums.APIType.Undefined && (!activity?.IsServiceLevelPickUp ?? false) && productOption.PickUpOption == Isango.Entities.PickUpDropOffOptionType.MultiplePickupLocation)
                                {
                                    aotEntity.PickupLocations = SerializeDeSerializeHelper.Serialize(productOption?.OptionPickupLocations);
                                }
                            }
                            catch (Exception ex)
                            {
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            DefaultAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new DefaultAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
