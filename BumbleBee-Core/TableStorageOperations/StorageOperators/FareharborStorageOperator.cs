...
{
    public class FareharborStorageOperator : BaseStorageOperator
    {
        /// <summary>
            ...
            if (productOptions?.Count > 0)
            {
                ...
                foreach (var productOption in productOptions)
                {
                    ...
                    foreach (var date in validDates)
                    {
                        ...
                        var basePriceAndAvailability = (FareHarborPriceAndAvailability)GetPriceAndAvailability(productOption.BasePrice, productOption.TravelInfo, date);
                        var costPriceAndAvailability = (FareHarborPriceAndAvailability)GetPriceAndAvailability(productOption.CostPrice, productOption.TravelInfo, date);
                        var gatePriceAndAvailability = (FareHarborPriceAndAvailability)GetPriceAndAvailability(productOption.GateBasePrice, productOption.TravelInfo, date);
                        productOption.TravelInfo.StartDate = date;
                        var rowKey = GetRowKey();

                        var fareharborEntity = new FareharborAvailabilities
                        {
                            UserKey = option.UserKey,
                            AvailToken = option.AvailToken,
                            CustomerTypePriceIds =
                                SerializeDeSerializeHelper.Serialize(basePriceAndAvailability.CustomerTypePriceIds),
                        ...
                        batchOperation.Insert(fareharborEntity);
                    }
                }
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            FareharborAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new FareharborAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
