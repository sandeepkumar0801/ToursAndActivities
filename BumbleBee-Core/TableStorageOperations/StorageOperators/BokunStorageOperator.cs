...
{
    public class BokunStorageOperator : BaseStorageOperator
    {
        /// <summary>
                ...
                foreach (var productOption in productOptions)
                {
                    ...
                    foreach (var date in validDates)
                    {
                        try
                        {
                            ...
                            var bokunEntity = new BokunAvailabilities
                            {
                                PricingCategoryId = SerializeDeSerializeHelper.Serialize(option.PricingCategoryId),
                                StartTimeId = option.StartTimeId,
                                RateId = option.RateKey,
                            ...
                            batchOperation.Insert(bokunEntity);
                        }
                        catch (Exception)
                        {
                            //Add logging here
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            BokunAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new BokunAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
