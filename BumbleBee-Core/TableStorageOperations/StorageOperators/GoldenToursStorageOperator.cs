...
{
    public class GoldenToursStorageOperator : BaseStorageOperator
    {
        /// <summary>
            ...
            if (productOptions?.Count > 0)
            {
                ...
                foreach (var productOption in productOptions)
                {
                    ...
                    foreach (var date in validDates)
                    {
                        ...
                        var goldenToursAvailabilities = new GoldenToursAvailabilities
                        {
                            ScheduleId = option.ScheduleId,
                            ProductType = option.ProductType,
                            RefNo = option.RefNo,
                            PickupLocations = SerializeDeSerializeHelper.Serialize(option.PickupLocations),
                            TransferOptions = SerializeDeSerializeHelper.Serialize(option.ContractQuestions),
                        ...
                        batchOperation.Insert(goldenToursAvailabilities);
                    }
                }
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            GoldenToursAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new GoldenToursAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
