...
{
    public class NewCitySightSeeingStorageOperator : BaseStorageOperator
    {
        ...
        public override void InsertAvailabilitiesData(Activity activity, string tokenId)
        {
            ...
            foreach (var productOption in activity.ProductOptions)
            {
                ...
                foreach (var date in validDates)
                {
                    ...
                    var gtEntity = new NewCitySightSeeingAvailabilities
                    {
                        ...
                        ContractQuestions = SerializeDeSerializeHelper.Serialize(actOpt?.ContractQuestions),
                        PartitionKey = tokenId,
                        RowKey = rowKey,
                        RateKey = actOpt.RateKey,//  "source": "cs_na",
                        RateCode = actOpt.Code,//"productCode": "cs_na",
                        VariantCondition= actOpt.VariantCondition
                    };
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            NewCitySightSeeingAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new NewCitySightSeeingAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
