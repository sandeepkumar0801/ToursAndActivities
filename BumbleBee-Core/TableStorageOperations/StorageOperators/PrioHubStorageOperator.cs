...
{
    public class PrioHubStorageOperator : BaseStorageOperator
    {
        /// <summary>
                ...
                if (productOptions?.Count > 0)
                {
                    foreach (var productOption in productOptions)
                    {
                        ...
                        foreach (var date in validDates)
                        {
                            ...
                            var basePriceAndAvailability = (PrioHubPriceAndAvailability)GetPriceAndAvailability(productOption.BasePrice, productOption.TravelInfo, date);
                            ...
                            var prioEntity = new PrioHubAvailabilities
                            {
                                ...
                                RowKey = rowKey,
                                PickupPointsDetails = SerializeDeSerializeHelper.Serialize(((ActivityOption)productOption)?.PickUpPointForPrioHub),
                                PrioHubProductTypeStatus = ((ActivityOption)productOption).PrioHubProductTypeStatus,
                                PrioHubProductPaxMapping = SerializeDeSerializeHelper.Serialize(((ActivityOption)productOption).PrioHubProductPaxMapping),
                                PrioHubProductGroupCode = ((ActivityOption)productOption).PrioHubProductGroupCode,
                                PrioHubAvailabilityId = basePriceAndAvailability?.AvailabilityId,
                                ProductCombiDetails = SerializeDeSerializeHelper.Serialize(((ActivityOption)productOption).ProductCombiDetails),
                                PrioHubComboSubProduct = SerializeDeSerializeHelper.Serialize(((ActivityOption)productOption).PrioHubComboSubProduct),
                                PrioHubClusterProduct = SerializeDeSerializeHelper.Serialize(((ActivityOption)productOption).Cluster),
                                PrioHubDistributerId = ((ActivityOption)productOption).PrioHubDistributerId,
                                CancellationText = productOption.CancellationText,
                                ApiCancellationPolicy = option.ApiCancellationPolicy,
                                TimeSlot =Convert.ToString(((ActivityOption)productOption)?.StartTime)
                            };
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            PrioHubAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new PrioHubAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
