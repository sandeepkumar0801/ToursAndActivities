...
{
    public class GlobalTixStorageOperator : BaseStorageOperator
    {
        /// <summary>
        ...
        public override void InsertAvailabilitiesData(Activity activity, string tokenId)
        {
            ...
            foreach (var productOption in activity.ProductOptions)
            {
                ...
                foreach (var date in validDates)
                {
                    ...
                    var gtEntity = new GlobalTixAvailabilities
                    {
                        ...
                        ContractQuestions = string.Empty,
        ...
        public override TableEntity Retrieve(string referenceId, string partitionKey)
        {
            ...
            GlobalTixAvailabilities AvailabilitiesEntityResolver(string pk, string rk, DateTimeOffset ts, IDictionary<string, EntityProperty> props, string etag)
            {
                var resolvedEntity = new GlobalTixAvailabilities() { PartitionKey = pk, RowKey = rk, Timestamp = ts, ETag = etag };
